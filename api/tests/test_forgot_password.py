...
async def test_issue_otp_stores_hashed_code_and_expiry():
    ...
    assert user.otp_expires_at <= datetime.now(timezone.utc) + timedelta(
        minutes=OTP_TTL_MINUTES + 1
    )
...
def test_forgot_password_endpoint_always_200_for_unknown_email():
    ...
    with patch(
        "app.routers.auth.get_user_by_email", new_callable=AsyncMock, return_value=None
    ):
...
def test_forgot_password_endpoint_issues_otp_for_known_email():
    ...
    with (
        ...
        patch(
            "app.routers.auth.issue_otp", new_callable=AsyncMock, return_value="654321"
        ) as mock_issue,
        ...
        client = TestClient(app)
        response = client.post(
            "/api/v1/auth/forgot-password", json={"email": "real@example.com"}
        )
